...
class _CustomCarouselSliderState extends State<CustomCarouselSlider> {
  ...
  Widget build(BuildContext context) {
    final List<Article> articlesWithImage = widget.articles
        .where((article) => article.tweetImage.isNotEmpty) // Filter for non-empty tweetImage
        .take(5) // Limit to 5 articles
        .toList(); // Convert to a new list
    final List<Widget> imageSliders =articlesWithImage
        .take(5)
        .map((article) => InkWell(
      onTap: () {
      ...
      child: Container(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            children: <Widget>[
              Image.network(article.tweetImage,
              ...
              Positioned(
                ...
                child: DecoratedBox(
                  ...
                  child: Padding(
                    ...
                    child: Text(
                      article.username, // Assuming username is the desired text
                      maxLines: 1, // Allow for 1 lines
                      overflow: TextOverflow.fade, // Truncate if content overflows
    ...
    return Column(
      children: [
        ...
        Row(
          ...
          children: List.generate(articlesWithImage.length, (index) {
...
class _CustomCarouselSliderState extends State<CustomCarouselSlider> {
  ...
  void initState() {
    super.initState();
    // Start the timer to update articles every 5 seconds
    _timer = Timer.periodic(const Duration(seconds: 5), (Timer timer) {
      setState(() {
        // Reset the current index to the first page
        _current = 0;
      });
    });
  }
    ...
    return Column(
      children: [
        CarouselSlider(
          ...
          carouselController: _controller,
          options: CarouselOptions(
            autoPlay: true, // Disable auto play to prevent conflicts with timer
            enlargeCenterPage: true,
            aspectRatio: 2.0,
            onPageChanged: (index, reason) {
              setState(() {
